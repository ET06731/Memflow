...
将散落在网页中的有价值内容——AI 对话、B 站视频、YouTube 视频、小红书卡片笔记、博客文章——一键剪藏到 Obsidian，配合 AI 智能摘要和高亮标注，让知识真正流动起来。
...
- ✅ **YouTube 视频智能摘要**：支持提取 YouTube 视频字幕，利用 AI 生成结构化摘要、关键词和分类（需要视频开启字幕功能）
...
1. 打开任意 YouTube 视频
2. **先开启字幕**：点击视频播放器右下角的 **CC** 按钮，选择字幕语言
3. **立即导出**：直接导出视频信息（含简介、字幕等基础内容）
4. **智能导出**：右键点击扩展按钮或 Shift+左键，自动提取字幕并利用 AI 生成结构化摘要

> ⚠️ 注意：YouTube 视频需要先开启字幕功能才能导出字幕内容
...
│   ├── adapters/       # B站、YouTube、ChatGPT、DeepSeek等平台适配器
